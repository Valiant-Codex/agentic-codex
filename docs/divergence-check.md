...
- **Linger + timers** — linger is on per agent, the host timers are enabled, and a
  `memory-mirror@<agent>.timer` is enabled for every roster agent (a never-enabled instance never
  runs, so it never fails either — no failed-unit sweep can see it).
- **Installed host artifacts** — everything `install-host-services` installed still matches its repo
  source (the manifest is generated by the installer itself, so its coverage cannot drift).
