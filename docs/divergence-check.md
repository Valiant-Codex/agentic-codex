...
- **Always-on context budget** — `CLAUDE.md` + its `@`-imports + the runtime auto-memory index,
  against `CONTEXT_BUDGET`. The index is injected by the runtime and lives outside the repo, so a
  budget watching only `CLAUDE.md` watches the cheaper half. The breakdown prints on every run: the
  index grows by normal operation, so the trend is the signal and the breach is already too late.
  If it breaches, shorten the index *hooks* — **never delete index lines.** A memory with no line in
  `MEMORY.md` is invisible to recall, which is a loss, not a saving.
