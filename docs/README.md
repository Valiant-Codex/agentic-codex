...
7. **[divergence-check.md](divergence-check.md)** — the structural linter that keeps brains honest: invariants, not vocabulary.
8. **[monitoring.md](monitoring.md)** — the dead-man's-switch: silence is the alarm.
9. **[secrets.md](secrets.md)** — secrets out of Git, in a store you can restore (Vaultwarden on Dokploy).
10. **[multi-agent-governance.md](multi-agent-governance.md)** — lanes, least privilege, and the honest limits of the separation.
11. **[app-layer.md](app-layer.md)** — the service layer (Dokploy / Cloudflare / apps): documented, not templated.
12. **[reference-architecture.md](reference-architecture.md)** — the real deployment this was distilled from.
