...
| `agent-audit/` | Interactive, human-in-the-loop tune-up of one agent's brain with <OWNER> (skills, its `CLAUDE.md` contract, memory). The periodic pass where accumulated experience becomes durable change; enforces that no scheduled job alters skills or identity. |
